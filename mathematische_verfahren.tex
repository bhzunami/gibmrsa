...
Es ist bei Primzahlen einfacher, die \textbf{teilerfremden} Zahlen zu zählen und diese dann von allen möglichen Zahlen abzuziehen, als alle nicht teilerfremden Zahlen zu zählen. Teilerfremd zu p sind $(p - 1) $ und für q gilt $ (q - 1) $. \cite{kryptologie} (Kapitel 5)\\
...
Der Satz von Euler sagt über zwei natürliche, teilerfremde Zahlen, hier $n$ und $m$, folgendes aus: \cite{kryptologie} (Kapitel 5) und \cite{zahlentheorie_fuer_einsteiger} (Kapitel 2)
%
\begin{equation}
  m^{\varphi(n)} \bmod(n) = 1
...
Somit stimmen die beiden grössten gemeinsamen Teiler überein\cite{zahlentheorie_fuer_einsteiger} (Kapitel 2).\\[2ex]
...
Der erweiterte Euklidische Algorithmus berechnet neben dem grössten gemeinsamen Teiler $r$ zweier natürlichen Zahlen a und b auch noch die dazugehörigen natürlichen Zahlen s und t, dass folgende Gleichung stimmt\cite{kryptologie} (Kapitel 5):
%
\begin{equation}
